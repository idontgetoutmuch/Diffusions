...
int main() {
  ...
  /* sunindextype ny = 120; */
  sunindextype nx = 6;        // spatial mesh size
  sunindextype ny = 12;
  ...
  dx = ONE/(ONE*nx-ONE);   // x mesh spacing
  dy = ONE/(ONE*ny-ONE);   // y mesh spacing

  sunindextype Ntot, ii, jj;
  Ntot = nx*ny;
  y = N_VNew_Serial(Ntot); // Create parallel vector for solution

  N_Vector h = NULL;
  h = N_VNew_Serial(Ntot);  // Create vector for heat source
  // fill in the heat source array
  data = N_VGetArrayPointer(h);
  for (jj=0; jj<ny; jj++)
    for (ii=0; ii<nx; ii++)
      data[IDX(ii,jj,nx)] = sin(PI*ii*dx)
                          * sin(TWO*PI*jj*dy);
  ...
  N_VLinearSum(ONE, y, ONE, h, y);

  realtype c1 = kx/dx/dx;
  realtype c2 = ky/dy/dy;
  realtype c3 = -TWO*(c1 + c2);
  printf("  %10.6"FSYM"  %10.6"FSYM"  %10.6"FSYM"\n", c1, c2, c3);
  for (jj=1; jj<ny-1; jj++)                        // diffusive terms
    for (ii=1; ii<nx-1; ii++)
      printf("  %10.6"FSYM"  %10.6"FSYM"  %10.6"FSYM"  %10.6"FSYM"  %10.6"FSYM"  %10.6"FSYM"\n",
	     Y[IDX(ii-1,jj,nx)], Y[IDX(ii+1,jj,nx)],
	     Y[IDX(ii,jj-1,nx)], Y[IDX(ii,jj+1,nx)],
	     Y[IDX(ii,jj,nx)],
	              c1*(Y[IDX(ii-1,jj,nx)] + Y[IDX(ii+1,jj,nx)])
	            + c2*(Y[IDX(ii,jj-1,nx)] + Y[IDX(ii,jj+1,nx)])
	            + c3*Y[IDX(ii,jj,nx)]);
