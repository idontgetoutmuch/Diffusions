...
data = np.loadtxt('heat2e.000.txt', dtype=np.double)
nt = np.shape(data)[0]
...
for tstep in range(nt):
    ...
    pname = 'heat2e_surf.' + repr(tstep).zfill(3) + '.png'
    cname = 'heat2e_err.' + repr(tstep).zfill(3) + '.png'
    tstr  = repr(tstep)
    nxstr = repr(nx)
    nystr = repr(ny)
    ...
    ax.view_init(20,45)
    plt.title('u(x,y) at output ' + tstr + ', mesh = ' + nxstr + 'x' + nystr)
    plt.savefig(pname)
    plt.close()
